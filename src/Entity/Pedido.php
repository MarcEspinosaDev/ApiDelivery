...
class Pedido
{
    ...
    /**
     * @ORM\OneToOne(targetEntity=Plato::class)
     * @ORM\JoinColumn(nullable=false)
     */
    private $platos;
     ...
     */
    private $platoCantidads;

    public function __construct()
    {
        $this->platoCantidads = new ArrayCollection();
    }
     ...
     */
    public function getPlatoCantidads(): Collection
    {
        return $this->platoCantidads;
    }

    public function addPlatoCantidad(PlatoCantidad $platoCantidad): self
    {
        if (!$this->platoCantidads->contains($platoCantidad)) {
            $this->platoCantidads[] = $platoCantidad;
            $platoCantidad->setPedido($this);
        }

        return $this;
    }

    public function removePlatoCantidad(PlatoCantidad $platoCantidad): self
    {
        if ($this->platoCantidads->removeElement($platoCantidad)) {
            // set the owning side to null (unless already changed)
            if ($platoCantidad->getPedido() === $this) {
                $platoCantidad->setPedido(null);
            }
        }
